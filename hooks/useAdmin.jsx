...
const useSearchIntegrator = (searchQuery) => {
  const [state, setState] = useState({
    data: [],
    viewData: {},
    userData: [],
  ...
  const handleFetchIntegrators = useCallback(
    async ({ pageIndex = 1, pageSize = 10, sortBy = [], searchQuery = '' }) => {
      ...
      try {
        const { data, success, errorMessage, totalCount } = await zat(INTEGRATOR.searchIntegrators, null, VERBS.GET, {
          page: pageIndex === 0 ? 1 : pageIndex,
          limit: pageSize,
          ...(sortField && { sortField }),
          ...(sortOrder && { sortOrder }),
...
export { useAdmin, useSearchIntegrator };
